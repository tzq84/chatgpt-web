...
router.post('/chat-process', [auth, limiter], async (req, res) => {
  ...
  try {
    ...
    let firstChunk = true
    console.log("chat request",{date:formatDate(new Date()),userid,name})
    ...
    if(process.env.OPENAI_API_DISABLE_DEBUG != "true") {
      console.log("chat response",{date:formatDate(new Date()),userid,name,prompt,completion:result&&result.data&&result.data.text?result.data.text:""})
    }
  }
  catch (error) {
